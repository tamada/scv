...
define __create_dist()
	...
	GOOS=$1 GOARCH=$2 go build -o dist/$(1)_$(2)/$(DIST)/$(NAME)$(3) main.go args.go printer.go
...
build: main.go args.go printer.go vector
	go build -o $(NAME) -v main.go args.go printer.go
